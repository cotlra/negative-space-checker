...
function scanProperty(
  ...
  for (const node of nodes) {
    ...
    if ("layoutMode" in node) {
      if (node.layoutMode == "HORIZONTAL" || node.layoutMode == "VERTICAL") {
        if (!variablesOfNode?.paddingTop) {
          paddingInfoArray.push(
            createInfo("paddingTop", node.paddingTop, node)
          );
        }
        if (!variablesOfNode?.paddingBottom) {
          paddingInfoArray.push(
            createInfo("paddingBottom", node.paddingBottom, node)
          );
        }
        if (!variablesOfNode?.paddingLeft) {
          paddingInfoArray.push(
            createInfo("paddingLeft", node.paddingLeft, node)
          );
        }
        if (!variablesOfNode?.paddingRight) {
          paddingInfoArray.push(
            createInfo("paddingRight", node.paddingRight, node)
          );
        }
        if (!variablesOfNode?.itemSpacing) {
          itemSpacingInfoArray.push(
            createInfo("itemSpacing", node.itemSpacing, node)
          );
        }
    ...
    if ("children" in node) {
      scanProperty(node.children, ignoreLocked);
    }
  }
}

function scanAll(nodes: ReadonlyArray<SceneNode>, ignoreLocked: boolean): void {
  clearArray();
  scanProperty(nodes, ignoreLocked);
...
figma.ui.onmessage = async (msg: {
  ...
  ignoreLocked: boolean | undefined;
}) => {
  if (msg.type === "scan-page") {
    ...
    scanAll(pageChildren, msg.ignoreLocked === true);
    ...
    scanAll(selection, msg.ignoreLocked === true);
    ...
    const nodeIds = [
      ...new Set(
        infoArray
          .filter((info) => info.value === msg.value)
          .map((info) => info.nodeId)
      ),
    ];
